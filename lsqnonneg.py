...
def lsqnonneg(C, d, x0=None, tol=None, itmax_factor=3, max_support=None,
              rel_err_thresh=0.01):
    ...
    while numpy.any(Z) and numpy.any(w[ZZ-1] > tol):
        ...
        num_pos = (x > 0).sum()
        rel_err = numpy.linalg.norm(resid) / numpy.linalg.norm(d)
        if num_pos % 10 == 0:
            print(" ... Active set has size {}".format(num_pos))
            print(" ...   with relative error of {}".format(rel_err))
        if rel_err < rel_err_thresh:
            print("\n ... Error threshold with relative error {} achieved. Exiting\n".format(rel_err))
            break
        if max_support is not None and num_pos >= max_support:
            print(("\n ... Maximum solution vector support reached with {} nonzero elements.  "
                   + "Exiting\n").format(num_pos))
            break
